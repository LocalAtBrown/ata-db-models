...
def pre_table_initialization(stage: Stage, components: list[Component], partner_names: list[Partner]) -> None:
    engine = create_engine(f"something something with admin access to default db")
    ...
    with engine.connect() as conn:
        ...
        for component in components:
            ...
            # TODO store creds per user in SSM
        conn.commit()


def initialize_tables(stage: str) -> None:
    pass


def post_table_initialization(stage: Stage, components: list[Component]) -> None:
    engine = create_engine(f"something with admin to THIS STAGE'S DB since we have to refer to tables")
